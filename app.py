...
class App:
    ...
    def init_gui(self):
        ...
        #extended
        self.btn_class_one = tk.Button(self.window, width=50, text="Extended", command=lambda: self.save_for_class(1))
        self.btn_class_one.pack(anchor=tk.CENTER, expand=True)

        #contracted
        self.btn_class_two = tk.Button(self.window, width=50, text="Contracted", command=lambda: self.save_for_class(2))
        self.btn_class_two.pack(anchor=tk.CENTER, expand=True)

        #button to train model
        self.btn_train = tk.Button(self.window, width=50, text="Train Model", command=lambda: self.model.train_model(self.counters))
        self.btn_train.pack(anchor=tk.CENTER, expand=True)

        self.btn_reset = tk.Button(self.window, width=50, text="Reset counter", command=self.reset)
